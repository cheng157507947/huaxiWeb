 ...
 *
 */
public class FieldHisDataViewExcel extends AbstractExcelView{
	...
	protected void buildExcelDocument(Map<String, Object> model, HSSFWorkbook workbook, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String[] title1 = { "采集时间","田块1水位（米）","田块2水位（米）","田块3水位（米）", "田块4水位（米）","土壤含湿量（%）","片区#1进水管瞬时流量（m3/h）","片区#1进水管累计流量（m3）","片区#2进水管瞬时流量（m3/h）","片区#2进水管累计流量（m3）","片区#3进水管瞬时流量（m3/h）","片区#3进水管累计流量（m3）"};
		...
		if(ListObject!=null){
			...
			for(CollHisData data: datas){
				...
				if(data.getData4()!=null){
					c.setCellValue(data.getData4());
				}
				
				c = r.createCell(5);
				c.setCellStyle(numCellStyle);
				c.getCellStyle().setVerticalAlignment(CellStyle.VERTICAL_CENTER);
				if(data.getData5()!=null){
					c.setCellValue(data.getData5());
				}
				
				c = r.createCell(6);
				c.setCellStyle(numCellStyle);
				c.getCellStyle().setVerticalAlignment(CellStyle.VERTICAL_CENTER);
				if(data.getData6()!=null){
					c.setCellValue(data.getData6());
				}
				
				c = r.createCell(7);
				c.setCellStyle(numCellStyle);
				c.getCellStyle().setVerticalAlignment(CellStyle.VERTICAL_CENTER);
				if(data.getData7()!=null){
					c.setCellValue(data.getData7());
				}
				
				c = r.createCell(8);
				c.setCellStyle(numCellStyle);
				c.getCellStyle().setVerticalAlignment(CellStyle.VERTICAL_CENTER);
				if(data.getData8()!=null){
					c.setCellValue(data.getData8());
				}
				
				c = r.createCell(9);
				c.setCellStyle(numCellStyle);
				c.getCellStyle().setVerticalAlignment(CellStyle.VERTICAL_CENTER);
				if(data.getData9()!=null){
					c.setCellValue(data.getData9());
				}
				
				c = r.createCell(10);
				c.setCellStyle(numCellStyle);
				c.getCellStyle().setVerticalAlignment(CellStyle.VERTICAL_CENTER);
				if(data.getData10()!=null){
					c.setCellValue(data.getData10());
				}
				
				c = r.createCell(11);
				c.setCellStyle(numCellStyle);
				c.getCellStyle().setVerticalAlignment(CellStyle.VERTICAL_CENTER);
				if(data.getData11()!=null){
					c.setCellValue(data.getData11());
				}
			}
		}
	}
