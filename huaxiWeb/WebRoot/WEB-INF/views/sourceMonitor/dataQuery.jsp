...
<LINK href="${ctx }/css/sourceMonitor/main.css" type="text/css" rel="stylesheet"/>  
...
function dataExport(){
	...
	var endTime = document.getElementById("endTime").value;	
	$.ajax({
		   url: "${ctx}/gm_historydata_verificationData.action", 
	       type: "POST",  
	       dataType: "json",//xml,html,script,json
	       data:{ch_id_list : channels,beginTime : beginTime,endTime : endTime},
	       success: function(data){
	    	   var is = data.message;
	    	   if(is == "true"){
	    		   var id = document.getElementById("sceneId").value;
					if(id != ""){
						window.location.href="${ctx}/gm_historydata_baseDataExport.action?ch_id_list="+channels+"&beginTime="+beginTime+"&endTime="+endTime+"&scene_id="+id;
					}
	    	   }else{
