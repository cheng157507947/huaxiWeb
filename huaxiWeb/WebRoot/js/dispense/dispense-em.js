...
function echoSceneTree(id,name,type,group){
	...
	if(hid_type!=null){
		if(hid_type.value != type){
			if(type == 1){
				this.location = ctx+"/admin/envirMonitor/realTimeField?id="+id; //田块
			}else if(type == 2){
				this.location = ctx+"/admin/envirMonitor/realTimePump?id="+id;//泵房
			}else if(type == 3){
				//this.location = "${param.ctx }/op_scene_other_two.action";//水质
			}else if(type == 4){
				this.location = ctx+"/admin/envirMonitor/realTimeWeather?id="+id;//气象
			}else{
